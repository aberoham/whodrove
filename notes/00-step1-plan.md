...
| Session recording → "Enhanced recording (BPF)" | implied BPF events are part of the recording stream | v17 emits BPF `SessionCommand` / `SessionDisk` / `SessionNetwork` through the audit emitter (`lib/bpf/bpf.go:424,483,538`), and the BPF context is wired with `s.emitter`, not `s.Recorder()` (`lib/srv/sess.go:1391-1401`) | `notes/01` "Things that are not in the audit log" and `notes/02` "Enhanced recording (BPF)" |
